...
`M.<product>.<option> = <value>`. For example:

	M.headers.path = include
	M.executable.path = bin/$(M.ARCH)
	M.executable.suffix = .exe
	M.shared_lib.path = lib/$(M.ARCH)
	M.shared_lib.suffix = .so
...
For example, an executable product might look something like this
	...
	M.product.exe.path = 
	M.product.exe.prefix = 
	M.product.exe.suffix = 
	M.product.exe.depends = $$($1.object)

	define M.product.exe.recipe
		gcc -o $@ $<
	endef

A zipfile product that creates a zip file from a bunch of spore sources might look like this

	M.product.zip.path = bin
	M.product.zip.prefix=
	M.product.zip.suffix=.zip
	M.product.zip.depends=$$($1.source)

	define M.product.zip.recipe
		zip -f $@ $<
	endef
